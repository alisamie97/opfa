...
class ModelCatalogProduct extends Model {
	...
	public function reGetProducts($data = array(),$total_filter_groups = 0){

	    if($total_filter_groups==0){
            ...
            $sql1 = "SELECT pf.product_id 
                    ...
                    HAVING COUNT(f.filter_group_id) = {$total_filter_groups}";
    ...
    //ali97rey edit:
    public function reGetTotalFilterGroups($filter_data){
        $sql = "SELECT COUNT(DISTINCT f.filter_group_id) as count_fg
                FROM oc_filter f
                JOIN oc_category_filter cf
                ON cf.filter_id = f.filter_id 
                WHERE f.filter_id IN ({$filter_data['filter_filter']}) AND cf.category_id = {$filter_data['filter_category_id']} ";
