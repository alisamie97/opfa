...
echo $sql;
	...
	//ali97rey edit:
    public function reGetFiltersOfFilterGroups($filters){
	    $sql = "SELECT f.filter_group_id AS filter_group ,GROUP_CONCAT(f.filter_id SEPARATOR ',') AS filters FROM oc_filter f WHERE f.filter_id IN ({$filters}) GROUP BY f.filter_group_id ";
